...
//! # The static body
//!
//! One body is held fixed. Without it the problem is singular in six directions, because rigidly
...
//! result is expressed in. It contributes no parameters, so a set of `n` bodies has `6 * (n - 1)`
//! of them.
//!
//! # Layout
//!
//! Parameters are laid out by body in index order, skipping the static one, six at a time. So with
//! four bodies and body 1 static, the vector is
//!
//! ```text
...
pub struct MultiAlignParams3 {
    ...
    bodies: Vec<AlignParams3>,

    /// The concatenated parameters of every non-static body.
    storage: DVector<f64>,
}

impl MultiAlignParams3 {
    /// Builds the parameterization from one [`AlignParams3`] per body.
    ///
    /// Use this when the bodies need individually chosen local origins, working offsets, or
    /// degree-of-freedom locks. [`MultiAlignParams3::from_centers`] covers the common case.
    ///
    /// Returns an error if there are fewer than two bodies, or if `static_i` is out of range.
    pub fn new(static_i: usize, bodies: Vec<AlignParams3>) -> Result<Self> {
        if bodies.len() < 2 {
            return Err(format!(
    ...
    pub fn from_centers(
        ...
        let bodies = centers
            ...
            .map(|(i, c)| {
                ...
                let start = initial.map_or_else(Iso3::identity, |t| t[i]);
    ...
    /// The column in the flat parameter vector where a body's six parameters begin, or `None` for
    /// the static body, which has none.
    ...
    pub fn body(&self, body: usize) -> &AlignParams3 {
        &self.bodies[body]
    }

    /// The current world transform of a body.
    pub fn transform(&self, body: usize) -> Iso3 {
...
mod tests {
    ...
    fn a_static_body_at_index_zero_shifts_everything_down() {
        let params = MultiAlignParams3::from_centers(0, &centers(), None, None).unwrap();
    ...
    fn initial_poses_are_reproduced_exactly_at_zero_parameters() {
        ...
        for _ in 0..500 {
            ...
            let params =
                MultiAlignParams3::from_centers(2, &centers, Some(&initial), None).unwrap();
    ...
    fn the_static_body_never_moves() {
        ...
        let mut params =
            MultiAlignParams3::from_centers(1, &centers, Some(&initial), None).unwrap();
    ...
    fn parameters_reach_the_body_they_belong_to() {
        let centers = centers();
        let mut params = MultiAlignParams3::from_centers(1, &centers, None, None).unwrap();
    ...
    fn locked_dof_are_enforced_on_every_body() {
        ...
        let mut params = MultiAlignParams3::from_centers(1, &centers, None, Some(dof)).unwrap();
    ...
    fn degenerate_inputs_are_rejected() {
        ...
        assert!(MultiAlignParams3::from_centers(0, &one, None, None).is_err());

        let centers = centers();
        assert!(MultiAlignParams3::from_centers(9, &centers, None, None).is_err());

        let short = vec![Iso3::identity(); 2];
        assert!(MultiAlignParams3::from_centers(0, &centers, Some(&short), None).is_err());
    }
}
