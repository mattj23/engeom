//! Options controlling the behavior of a 3D alignment.

/// Options controlling how a 3D points-to-surface alignment weighs and filters its
/// correspondences.
///
/// Construct with [`AlignOptions3::default`] and override the fields you care about, so that
/// options added later don't break existing call sites.
///
/// # Robust estimation
///
/// By default, the alignment performs [`AlignOptions3::refinement_steps`] rounds of iteratively
...
///
/// Set `refinement_steps` to `0`, or use [`AlignOptions3::no_refine`], to get a plain unweighted
/// least-squares alignment.
#[derive(Clone, Copy, Debug)]
pub struct AlignOptions3<'a> {
    /// If the surface target can tell that a point does not project directly onto the target
    /// (such as when it projects past the edge of a bounded surface), setting this flag weights
    /// such points at 0.0 to prevent their influence on the alignment.
    ...
    /// `AlignSurfMatch3::sigma`), the two combine in quadrature as `sqrt(test^2 + target^2)`,
    /// which is the standard deviation of the difference of two independent measurements.
    ///
    /// Every entry must be finite and strictly positive. `Mesh3::point_stdev` is what provides
    /// this for a point set sampled from a measured mesh.
    ...
    /// alignment re-establishes its correspondences on every step, so an unconverged result near
    /// an edge or a corner is often a correspondence flipping back and forth rather than a budget
...
/// [`AlignOptions3::default`] does not silently drift if the crate changes it.
const DEFAULT_PATIENCE: usize = 100;

impl AlignOptions3<'_> {
    /// Options with robust refinement disabled, giving a plain unweighted least-squares alignment.
...
impl Default for AlignOptions3<'_> {
    fn default() -> Self {
        Self {
