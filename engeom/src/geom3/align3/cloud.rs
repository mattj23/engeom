...
mod tests {
    ...
    fn cloud_target_rejects_a_nonsense_extrapolation() {
        let cloud = PointCloud3::from_surface_points(&engine_blade().sample_dense(1.0, None));
    ...
    fn cloud_target_takes_its_weight_from_voxel_coherence() {
        ...
        let cloud = PointCloud3::from_surface_points(&mesh.sample_dense(0.5, None))
            .reduce_by_voxel(2.0)
            .unwrap();
    ...
    fn the_tangent_plane_projection_is_what_removes_the_sampling_floor() {
        ...
        let test_points: Vec<Point3> = mesh
            .sample_poisson(1.0, None)
            .iter()
            .map(|m| m.sp.point)
            .collect();
        ...
        let spacing = 2.0;
        let cloud = PointCloud3::from_surface_points(&mesh.sample_dense(spacing, None));
