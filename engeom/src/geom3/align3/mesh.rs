...
pub fn generate_alignment_points(
    ...
    let all_points = test_mesh.sample_poisson(sample_spacing);
    ...
    if let Some(sigma) = filter_distances {
...
fn smpl_check(
    ...
    for sp in &neighbors_ref {
        if dist(sp, &check_ref) < max_spacing {
            return (false, f64::MAX);
        }
    }
...
pub fn sac_check(
    ...
    for n in neighbors.iter() {
        if dist(n, check_point) > max_spacing * 2.0
            || n.sp.normal.angle(&check_point.sp.normal) > max_neighbor_angle
            || check_point.sp.scalar_projection(&n.sp.point).abs() > max_spacing
        {
            return false;
        }
    }
    ...
    if points
        .iter()
        .map(|p| p.z.abs())
        .any(|z| z > out_of_plane_ratio * max_spacing)
    {
        return false;
    }
    ...
    if dist(&centroid, &check2) > max_spacing * centroid_ratio {
        return false;
    }
