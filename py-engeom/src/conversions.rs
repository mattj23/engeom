...
pub fn points2_to_array2(points: &[Point2]) -> Array2<f64> {
    let mut array = Array2::zeros((points.len(), 2));
    for (i, point) in points.iter().enumerate() {
        array[[i, 0]] = point.x;
        array[[i, 1]] = point.y;
    }
...
pub fn faces_to_array2(faces: &[[u32; 3]]) -> Array2<u32> {
...
pub fn array2_to_faces(array: &ArrayView2<'_, u32>) -> PyResult<Vec<[u32; 3]>> {
...
pub fn array2_to_vectors3(array: &ArrayView2<'_, f64>) -> PyResult<Vec<Vector3>> {
    let shape = array.shape();
    if shape.len() != 2 || shape[1] != 3 {
...
pub fn array2_to_vectors2(array: &ArrayView2<'_, f64>) -> PyResult<Vec<Vector2>> {
    let shape = array.shape();
    if shape.len() != 2 || shape[1] != 2 {
